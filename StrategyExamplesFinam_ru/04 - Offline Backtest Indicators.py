...
if __name__ == '__main__':
    ...
    ticker = 'TQBR.SBER'  # Тикер в формате <Код тикера>
    ticker2 = 'TQBR.LKOH'  # Тикер в формате <Код тикера>
    ...
    # data = store.getdata(timeframe=bt.TimeFrame.Minutes, compression=10, dataname=ticker, fromdate=fromdate, live_bars=False)  # поставьте здесь True - если нужно получать live бары
    # data2 = store.getdata(timeframe=bt.TimeFrame.Minutes, compression=10, dataname=ticker2, fromdate=fromdate, live_bars=False)  # поставьте здесь True - если нужно получать live бары

    timeframe = "M5"
    fromdate = dt.datetime.utcnow() - dt.timedelta(minutes=5 * 1000)
    data = store.getdata(dataname=ticker, timeframe=bt.TimeFrame.Minutes, compression=5, fromdate=fromdate, live_bars=False,
    ...
    data2 = store.getdata(dataname=ticker2, timeframe=bt.TimeFrame.Minutes, compression=5, fromdate=fromdate,
                         live_bars=False,
                         super_candles=True,  # для получения свечей SuperCandles с расширенным набором характеристик
                         metric='tradestats',  # + необходимо указать тип получаемых метрик
                         )
    ...
    # data = store.getdata(timeframe=bt.TimeFrame.Days, compression=1, dataname=ticker, fromdate=fromdate, live_bars=False)  # поставьте здесь True - если нужно получать live бары
    # data2 = store.getdata(timeframe=bt.TimeFrame.Days, compression=1, dataname=ticker2, fromdate=fromdate, live_bars=False)  # поставьте здесь True - если нужно получать live бары
