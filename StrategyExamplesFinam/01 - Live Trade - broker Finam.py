...
class RSIStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        for data in self.datas:  # We run through all the requested bars of all tickers
            ...
            _point = ticker.index('.')
            board = ticker[0:_point]  # ticker class
            symbol = ticker[_point + 1:]  # ticker +1 for point
...
def get_some_info_for_tickers(tickers, live_prefix):
    ...
    for ticker in tickers:
        _point = ticker.index('.')
        board = ticker[0:_point]  # class of ticker
        symbol = ticker[_point + 1:]  # ticker +1 for point
        # print(board, symbol)
...
if __name__ == '__main__':
    ...
    security_board = "TQBR"  # class of ticker
    symbol = 'SBER'  # ticker
    # symbol2 = 'LKOH'  # ticker
    ...
    info_tickers = get_some_info_for_tickers([ticker, ], live_prefix)  # we take information about the ticker (minimum price step, number of decimal places)
    ...
    data = store.getdata(timeframe=bt.TimeFrame.Minutes, compression=1, dataname=ticker, fromdate=fromdate,
                         live_bars=True, name=f"{live_prefix}{ticker}")  # set True here - if you need to receive live bars # name - you need it to submit live applications
